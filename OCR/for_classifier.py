...
def format_result(df):
    ...
    # # 1) Приводим обе колонки к числовому типу (строки → NaN)
    # df["Количество"] = pd.to_numeric(df["Количество"], errors="coerce")
    # df["Цена"] = pd.to_numeric(df["Цена"], errors="coerce")
    #
    # # 2) Удаляем строки без валидных чисел или с нулевым/отрицательным количеством
    ...
    # # 3) Делаем деление
    # df["Цена за 1 шт."] = df["Цена"] / df["Количество"]
    #
    # return df
